...
const MODELS = [
  { name: 'Auto', tokenizer: 'auto-detect', note: 'Best guess based on content' },
  ...
  return (
    <div className="tab-panel presets-tab">
      <section className="preset-section">
        ...
        <p className="section-desc">Choose a preset that matches your accuracy vs. size trade-off.</p>
        {presets ? (
          <div className="table-wrap glass">
            <table className="data-table preset-table">
              <thead><tr><th>Preset</th><th>Target reduction</th><th>Dedup threshold</th><th>Accuracy floor</th><th>Description</th></tr></thead>
              <tbody>
                {Object.entries(presets).map(([key, p]) => (
                  <tr key={key}><td className="mono cap">{key}</td><td>{Math.round((p.target_compression ?? 0) * 100)}%</td><td>{p.dedup_threshold ?? '—'}</td><td>{p.accuracy_floor ?? '—'}%</td><td>{p.description ?? '—'}</td></tr>
                ))}
              </tbody>
            </table>
          </div>
      ...
      <section className="preset-section">
        ...
        <div className="table-wrap glass">
          <table className="data-table model-table">
            <thead><tr><th>Model</th><th>Tokenizer</th><th>Note</th></tr></thead>
            <tbody>
              {MODELS.map((m) => (<tr key={m.name}><td className="mono">{m.name}</td><td className="mono">{m.tokenizer}</td><td>{m.note}</td></tr>))}
            </tbody>
          </table>
        </div>
      </section>
