...
export default function DiffTab() {
  ...
  const handleDiffFile = useCallback((file) => {
    ...
    reader.onload = (e) => { const text = e.target.result; setDiffText(text); parseDiffFiles(text) }
  ...
  const parseDiffFiles = (text) => {
    ...
    const init = {}; files.forEach((f) => { init[f] = diffFileContents[f] || '' })
    setDiffFileContents((prev) => ({ ...prev, ...init }))
  }

  const updateFileContent = (path, value) => setDiffFileContents((prev) => ({ ...prev, [path]: value }))

  const runCompress = useCallback(async () => {
    setDiffLoading(true); setDiffError(null); setDiffResult(null)
    try {
      const data = diffMode === 'github'
        ? await compressDiffGithub({ pr: diffPrUrl, model })
        : await compressDiff({ diff_text: diffText, file_contents: diffFileContents, model })
      setDiffResult(data)
    } catch (err) {
      setDiffError(err.message === 'Failed to fetch' ? "Can't reach the API" : err.message)
      ...
      <div className="workspace-columns">
        <div className="column glass glass-hover">
          ...
          <div className="diff-mode-toggle">
            ...
            <button className={`seg-btn ${diffMode === 'github' ? 'active' : ''}`} onClick={() => setDiffMode('github')}>GitHub PR</button>
          </div>
          {diffMode === 'manual' ? (
            <>
              <textarea className="code-input" placeholder="Paste unified diff here…" value={diffText} onChange={(e) => { setDiffText(e.target.value); parseDiffFiles(e.target.value) }} spellCheck={false} />
              ...
              {parsedFiles.length > 0 && (
                <div className="file-contents-list">
                  <div className="panel-subheader">Provide new file contents for changed files</div>
                  {parsedFiles.map((path) => (
                    <div key={path} className="file-content-card">
                      <label className="file-path">{path}</label>
                      <textarea className="code-input mini" placeholder={`Paste new content for ${path}`} value={diffFileContents[path] || ''} onChange={(e) => updateFileContent(path, e.target.value)} spellCheck={false} />
                    </div>
                  ))}
                </div>
              )}
          ...
          {diffResult ? (
            <div className="results-stack">
              ...
              {diffResult.files_skipped && diffResult.files_skipped.length > 0 && (
                <div className="alert warn">Skipped files (no content provided): {diffResult.files_skipped.join(', ')}</div>
              )}
              ...
              {diffResult.notes && diffResult.notes.length > 0 && (
                <details className="notes-details glass"><summary>Pipeline notes ({diffResult.notes.length})</summary><ul>{diffResult.notes.map((n, i) => <li key={i}>{n}</li>)}</ul></details>
              )}
            </div>
