...
export default function TextTab() {
  ...
  const runCompress = useCallback(async () => {
    ...
    try {
      ...
      let data
      if (textFile) {
        data = await compressFile(textFile, payload)
      } else {
        data = await compressText({ ...payload, text: textInput })
      }
      setTextResult(data)
    } catch (err) {
        ...
        <div className="column input-col">
          ...
          {textFile && (
            <div className="file-chip">
              ...
              <button onClick={() => setTextFile(null)}>×</button>
            </div>
          )}
          ...
          <div className="controls-stack">
            ...
            <label className={`control-row slider-row ${!isCustom ? 'disabled' : ''}`}>
              ...
              <input
                ...
                max={95}
                step={5}
                value={Math.round(textTarget * 100)}
                onChange={(e) => setTextTarget(Number(e.target.value) / 100)}
                disabled={!isCustom}
              />
            </label>
            ...
            <button
              ...
              onClick={runCompress}
              disabled={textLoading || (!textInput.trim() && !textFile)}
            >
          ...
          {textResult ? (
            <div className="results-stack">
              ...
              <div className="bar-chart">
                <div
                  className="bar-reduced"
                  style={{ width: `${(textResult.compression_ratio ?? 0) * 100}%` }}
                />
              </div>
              ...
              <div className="result-actions">
                <button
                  className="btn-ghost"
                  onClick={() => navigator.clipboard.writeText(textResult.compressed_text)}
                >
                  Copy output
                </button>
              </div>
              ...
              <textarea
                ...
                readOnly
                value={textResult.compressed_text ?? ''}
              />
              ...
              {textResult.notes && textResult.notes.length > 0 && (
                <details className="notes-details">
                  <summary>Pipeline notes ({textResult.notes.length})</summary>
                  <ul>
                    {textResult.notes.map((n, i) => (
                      <li key={i}>{n}</li>
                    ))}
                  </ul>
                </details>
